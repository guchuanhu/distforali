...
;(function($, global){
    ...
    zValidate.prototype = {
		...
		methods: {//验证方法 可自定义扩展
			...
			mobile: function(value, $element, param) {//手机
				return this.optional($element) || /^[1][3,4,5,7,8][0-9]{9}$/.test(value);
			},
			phone: function(value, $element, param) {//固定电话
				return this.optional($element) || /^([0-9]{3,4}-)?[0-9]{7,8}$/.test(value);
			},
			post: function(value, $element, param) {//邮政编码
